...
def define_props():
    ...
    item = []
    for i in range(1, 12):
        item.append((str(i), "Collision Group " + str(i), "collides only with group " + str(i)))
    ...
    parset.mol_link_rellength = bpy.props.BoolProperty(name="mol_link_rellength", description="Activate search distance relative to particle's radius", default=True)
    ...
    parset.mol_link_max = bpy.props.IntProperty(name="mol_link_max", description="Maximum of links per particle", min=0, default=16)
...
class MolSimulate(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        print("Molecular Simumation Starting--------------------------------------------------")
        mol_stime = clock()
        scene = bpy.context.scene
...
class MolSimulateModal(bpy.types.Operator):
    ...
    def modal(self, context, event):
        ...
        if event.type == 'ESC' or frame_current == frame_end:
            ...
            cmolcore.memfree()
            mol_simrun = False
            print("--------------------------------------------------Molecular Simulation Finished!")
